...
<div class="p-6">
    ...
    <p class="font-poppins text-gray-400">Pembuatan akun user atau admin</p>
</div>
<div class="px-6 py-2">
    <div class="bg-white rounded-md border border-gray-100 shadow-black/5 p-6">
        <h1 class="font-semibold text-xl mb-4">Buat Akun</h1>
        <form method="post" action="{{ route('user.store') }}" enctype="multipart/form-data"> <!-- Tambahkan enctype untuk mengizinkan upload file -->
            @csrf
            <fieldset>
                ...
                <label for="role" class="block mb-6">
                    ...
                    <select name="role" id="role" class="mt-1 px-3 py-2 bg-gray-100 border shadow-sm border-slate-200 placeholder-slate-400 focus:outline-none focus:border-gray-300 focus:ring-gray-300 block w-full rounded-md sm:text-sm focus:ring-1" required>
                        <option value="" disabled selected>Pilih Role</option> <!-- Opsi default -->
